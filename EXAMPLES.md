...
const maskedUrl =
	'steam://rungame/730/76561202255233023/+csgo_econ_action_preview%20M[U:1:123456]A45540551473D7523863830991655521';
...
cs2
	.inspectItem(owner, assetid, classid)
	.then((item) => {
		console.log('Item:', item);
	})
	.catch((err) => {
...
cs2.on('itemCustomizationNotification', (itemIds, notificationType) => {
	const typeName = Object.keys(NodeCS2.ItemCustomizationNotification).find(
		(key) => NodeCS2.ItemCustomizationNotification[key] === notificationType
	);
...
// Request profile
cs2.requestPlayersProfile('76561198057249394').then((profile) => {
